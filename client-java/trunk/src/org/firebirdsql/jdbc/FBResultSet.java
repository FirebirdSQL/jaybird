 ...
 * the value of the first matching column will be ret
rned.
 * The column name op
ion is
 * designed to be used when column names are used in
he SQL
 * query that generated the resu
t set.
 * For columns that are NOT explicitly named in the qu
ry, it
 * is best to use column numbers. If column names are used, t
ere is
 * no way for the programmer to guarantee that they actually r
fer to
 * the intended c
lumns.
 * <P>
 * A set of <code>updateXXX</code> methods were added to this in
erface
 * in the JDBC 2.0 API (Java<sup><font size=-2>TM</font></sup>
2 SDK,
 * Standard Edition, version 1.2). The comments regarding par
meters
 * to the <code>getXXX</code> methods also apply to parameters
to the
 * <code>updateXXX</code> m
thods

 *<P>
 * The <code>updateXXX</code> methods may be used in tw
 ways:

* <ol>
 * <LI>to update a column value in the current row.  In a scr
llable
 *     <code>ResultSet</code> object, the cursor can be moved ba
kwards
 *     and forwards, to an absolute position, or to a p
sition
 *     relative to the curre
t row.
 *     The following code fragment updates the <code>NAME</code>
column
 *     in the fifth row of the <code>ResultSet</code>
object
 *     <code>rs</code> and then uses the method <code>updateRow
/code>
 *     to update the data source table from which <code>rs</code> was d
rived.

 <
RE>
 *
 *       rs.absolute(5); // moves the cursor to the fifth ro
 of rs
 *       rs.updateString("NAME", "AINSWORTH"); // updat
s the
 *          // <code>NAME</code> column of row 5 to be <code>AINSWORTH
/code>
 ...
 * <LI>to insert column values into the insert row.  An up
atable
 *     <code>ResultSet</code> object has a special row associat
d with
 *     it that serves as a staging area for building a row to be in
erted.
 *     The following code fragment moves the cursor to the insert row,
builds
 *     a three-column row, and inserts it into <code>rs</code> a
d into
...
RE>
 *
 *       rs.moveToInsertRow(); // moves cursor to the ins
rt row
 *       rs.updateString(1, "AINSWORTH"); // updat
s the
 ...
 * <P>A <code>ResultSet</code> object is automatically closed w
en the
 * <code>Statement</code> obje
t that
 * generated it is closed, re-executed,
r used
 * to retrieve the next result from a sequence of multiple r
sul
s.
 *
...
/code>
 * object returned by the <code>ResultSet.getMetaData</code>
et
od.
 *
 * @see Statement#execut
Query
 * @see Statement#getRes
ltSet
 * @see ResultSetMe
aDa

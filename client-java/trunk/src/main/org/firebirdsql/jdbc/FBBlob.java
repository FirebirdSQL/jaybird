...
public class FBBlob implements FirebirdBlob, Synchronizable {
    ...
    public byte[] getInfo(byte[] items, int buffer_length) throws SQLException {
        ...
        synchronized(syncObject) {
            try {
                ...
                isc_blob_handle blob = gdsHelper.openBlob(blob_id, SEGMENTED);
                try {
   ...
   */
    ...
    private long interpretLength(byte[] info, int position) throws SQLException { 
        ...
        int dataLength = 
            gdsHelper.getInternalAPIHandler().isc_vax_integer(info, position + 1, 2);
            
        return gdsHelper.getInternalAPIHandler().isc_vax_integer(
    ...
    public boolean isSegmented() throws SQLException {
        ...
        int dataLength =
            gdsHelper.getInternalAPIHandler().isc_vax_integer(info, 1, 2);

        int type = gdsHelper.getInternalAPIHandler().isc_vax_integer(
            info, 3, dataLength);
        ...
        implements FirebirdBlob.BlobInputStream
    {
        ...
        public void seek(int position, int seekMode) throws IOException {
            ...
            synchronized(syncObject) {
                checkClosed();
                try {
                    gdsHelper.getInternalAPIHandler().isc_seek_blob(blob, position, seekMode);
                } catch (GDSException ex) {
                    /** @todo fix this */
        ...
        public long length() throws IOException {
            ...
            synchronized(syncObject) {
                checkClosed();
                try {
                    byte[] info = gdsHelper.getInternalAPIHandler().isc_blob_info(

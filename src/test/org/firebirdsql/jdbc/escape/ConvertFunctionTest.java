...
class ConvertFunctionTest {
    ...
    void testZeroParameters_throwsException() {
        assertParseException(function::apply, "Expected 2 parameters for CONVERT, received 0");
    }

    @Test
    void testOneParameter_throwsException() {
        assertParseException(() -> function.apply("val"), "Expected 2 parameters for CONVERT, received 1");
    }

    @Test
    void testThreeParameter_throwsException() {
        assertParseException(() -> function.apply("val", "BIGINT", "XYZ"),
                "Expected 2 parameters for CONVERT, received 3");
    }

    @ParameterizedTest(name = "{index}: convert({0}, {1}) : {2}")
    @MethodSource("convertTestCases")
    void testConvert(String value, String dataType, String expectedResult) throws Exception {
        assertEquals(expectedResult, function.apply(value, dataType));
    }

    public static Stream<Arguments> convertTestCases() {
        return Stream.of(
//@formatter:off
...
//@formatter:on
    ...
    private static Arguments testCase(String value, String dataType, String expectedResult) {
        return Arguments.of(value, dataType, expectedResult);
    }

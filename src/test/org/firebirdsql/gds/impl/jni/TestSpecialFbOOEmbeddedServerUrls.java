...
public class TestSpecialFbOOEmbeddedServerUrls {

    @ClassRule
    public static final GdsTypeRule testType = GdsTypeRule.supports(FbOOEmbeddedGDSFactoryPlugin.EMBEDDED_TYPE_NAME);
    ...
    public void testDriverManagerGetConnectionWithoutServerAndPortInUrl() throws Exception {
        Connection connection = DriverManager.getConnection("jdbc:firebirdsql:fboo:embedded:" + mAbsoluteDatabasePath +"?encoding=NONE",
                "SYSDBA", "masterkey");
        connection.close();
    }
    ...
    public void testDriverManagerGetConnectionWithoutServerAndPortInUrlWithRelativeDatabasePath() throws Exception {
        ...
        try {
            ...
            Connection connection =
                    DriverManager.getConnection("jdbc:firebirdsql:fboo:embedded:" + mRelativeDatabasePath + "?encoding=NONE",
                            "SYSDBA", "masterkey");
            connection.close();
        } finally {
            testFBManager.stop();

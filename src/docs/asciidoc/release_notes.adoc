...
This release is not fully compatible with the upcoming JDBC 4.5 Specification.
...
We limit support to Java LTS releases (Java 17, Java 21, and Java 25), and the latest Java release.
Currently, that means we support Java 17, Java 21, and Java 25.
...
Java 26 will introduce https://jcp.org/aboutJava/communityprocess/maintenance/jsr221/index5.html[JDBC 4.5^].
...
The value of `JaybirdTypeCodes.DECFLOAT` will be updated in the first Jaybird 6 release _after_ the release of Java 26 (in March 2026).
Support for `JDBCType.DECFLOAT` instead of `JaybirdType.DECFLOAT` will also be added in that release.
...
For the time being, we recommend using the equivalent methods (with the same name) on a `java.sql.Statement` instance of the connection.

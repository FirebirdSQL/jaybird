...
Java 17 and higher supported using the Java 17 driver.
...
For "`ChaCha64`" wire-encryption support, also add `chacha64-plugin-{version_wo_target}.jar` and `bcprov-jdk18on-{bc-version}.jar`.
...
If you use native or embedded, make sure to update your JNA dependency to version {jna-version}, or remove it altogether as the dependencies will now be pulled in through the `jaybird-native` artifact.
...
* Fixed: `FBResultSetMetaData.getPrecision` would always estimate the precision of `NUMERIC` or `DECIMAL` columns instead of obtaining the actual precision if the column position was 71 or higher (https://github.com/FirebirdSQL/jaybird/issues/731[jaybird#731])
...
In general, we expect the driver to remain functional, if the protocol version is explicitly enabled, but chances are certain metadata -- e.g. `DatabaseMetaData` -- will break if we use features introduced in newer versions.

By default, Jaybird will only connect to Firebird 3.0 and higher.

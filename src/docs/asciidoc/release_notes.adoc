...
* Two system properties were added to disable forced close of a connection after fatal errors (issue:[893])
+
See also <<disable-force-close-on-fatal>>.
* Fixed: Legacy authentication fails with non-ASCII characters in password (issue:[858])
+
...
* Fixed: Off-by-one error in default value for `maxInlineBlobSize` (issue:[937])
+
Fix was contributed by Artyom Abakumov.
* Fixed: `NullPointerException` on authentication failure with non-existent user and single plugin in `AuthServer` (issue:[940])
...
* Dependency update: updated `org.bouncycastle:bcprov-jdk18on` from 1.83 to 1.85.2 (used by `chacha64-plugin`) (issue:[957])
...
* JDBC 4.5 support: `JaybirdTypeCodes.DECFLOAT` and `JaybirdType.DECFLOAT` now use type code `2015` instead of `-6001` (issue:[906])
+
...
* JDBC 4.5 support: implemented methods `enquoteIdentifier`, `enquoteLiteral`, `enquoteNCharLiteral`, and `isSimpleIdentifier` on `FBConnection`, and added them to interface `FirebirdConnection` for access in older Java versions (issue:[908])
+
...
* JDBC 4.5 support: implemented "`disable escape processing`" JDBC escape (`++{\...\}++`) (issue:[909])
+
See also <<jdbc-escape-disable-proc>>.
* JDBC 4.5 support: `FBDatabaseMetaData.getJDBCMinorVersion()` should report 5 (for JDBC 4.5) on Java 26 and higher (issue:[915])
* Fixed: JDBC escapes should not be parsed inside dialect 3 delimited identifiers or dialect 1 string literals (issue:[921])
* Fixed: `IndexOutOfBoundsException` in `FBCachedBlob.getBytes(long, int)` for position or length beyond end of data (issue:[923])
* Fixed: Using native client, password is limited to 255 bytes (issue:[925])
* Fixed: Infinite loop in `FBPooledConnection#fireConnectionError(SQLException)` if the exception has a chained exception and neither is fatal (issue:[927])
...
* Fixed: Statement close of a leaked statement by the cleaner did not detect fatal connection errors (issue:[879])
+
This change also fixes a `NullPointerException` when the connection was terminated due to a fatal connection error.
A minor breaking change was made in the internal GDS-ng wire API, which should only be relevant to maintainers of Jaybird forks, see <<breaking-changes-internal-api-6-0-4>>.
* Fixed: `Statement.cancel()` causes lockup (issue:[892])
+
This is a partial backport from Jaybird 7.
See also <<breaking-changes-internal-api-6-0-4>>.
* Fixed: Negative buffer size on Firebird 2.5 if information response is 32KiB or greater (issue:[895])
+
...
* Fixed: `FBServiceManager.getAuthPlugins()` reported the `dbCryptConfig` value (issue:[901])
* Dependency update: updated `net.java.dev.jna:jna-jpms` from 5.17.0 to 5.18.1 (used by `jaybird-native`) (issue:[910])
* Dependency update: updated `org.bouncycastle:bcprov-jdk18on` from 1.81 to 1.83 (used by `chacha64-plugin`) (issue:[912])
* Fixed: `FBDatabaseMetaData.getJDBCMinorVersion()` should report 4 (for JDBC 4.4) on Java 24 and higher (issue:[913])

=== Jaybird 6.0.3

The following was fixed or changed since Jaybird 6.0.2:

* Fixed: statement close could cause a hang of the connection (issue:[876])
+
...
* Fixed: `ResultSet` move incorrectly closes input `Clob` (issue:[880])
* Fixed: Batch execution with multiple empty strings resulted in error _"Repeated blob id 0:0 in registerBlob()"_ (issue:[888])
* Dependency update: updated `org.bouncycastle:bcprov-jdk18on` from 1.80 to 1.81 (used by `chacha64-plugin`) (issue:[889])
* Fixed: On Java 24, `Connection.abort`, `Connection.setNetworkTimeout`, and `OperationMonitor.initOperationAware` always throw _"java.lang.SecurityException: checking permissions is not supported"_ (issue:[890])

=== Jaybird 6.0.2

The following was fixed or changed since Jaybird 6.0.1:

* Improvement: added `authPlugins` property on `FBManager` (issue:[866])
+
...
* Improvement: increased default and maximum SQL info sizes used for retrieving statement information like columns, parameters and plan information (issue:[869])
+
...
* Improvement: backported inline blob support (Firebird 5.0.3 and higher) from Jaybird 7 (issue:[870])
+
See also <<blob-performance-inline-blob>>.
* Improvement: The time zone mapping was updated (issue:[874])
...
* Improvement: backported deferred blob open optimization from Jaybird 7 (issue:[842])
+
...
* Dependency update: updated `net.java.dev.jna:jna-jpms` from 5.16.0 to 5.17.0 (used by `jaybird-native`) (issue:[854])
* Dependency update: updated `org.bouncycastle:bcprov-jdk18on` from 1.79 to 1.80 (used by `chacha64-plugin`) (issue:[856])

=== Jaybird 6.0.0

The following was fixed or changed after 6.0.0-beta-1:

* Improvement: stacktraces produced by `FbExceptionBuilder` no longer include `StackTraceElements` of the builder itself (issue:[827])
* Improvement: support for custom SocketFactory in connection string and data sources (issue:[828])
+
See also <<custom-socket-factory>>.
* Improvement: added connection property `asyncFetch` and system property `org.firebirdsql.jdbc.defaultAsyncFetch` to disable async fetching for pure Java connections (issue:[831])
+
...
Jaybird {version_wo_target} was tested against Firebird 3.0.14, Firebird 4.0.7, and Firebird 5.0.4, but should also support other Firebird versions from 3.0 and up.
...
ChaCha64 wire encryption implementation (`chacha64-plugin-{version_example}.jar`) using https://www.bouncycastle.org/[Bouncy Castle^]
...
The NATIVE (JDBC URL prefix `++jdbc:firebird[sql]:native:++`/`++jdbc:firebird[sql]:local:++`) and EMBEDDED (JDBC URL prefix `++jdbc:firebird[sql]:embedded:++`) protocol implementations have been moved to a separate artifact, `org.firebirdsql.jdbc:jaybird-native`.
...
Given this requires a dependency on the https://www.bouncycastle.org/[Bouncy Castle^] provider (`bcprov-jdk18on`), the plugin is made available as a separate artifact: `org.firebirdsql.jdbc:chacha64-plugin` on Maven or `chacha64-plugin-{version_example}.jar` in the distribution zip.
...
Firebird 3.0 added https://firebirdsql.org/file/documentation/release_notes/html/en/3_0/rlsnotes30.html#rnfb30-psql-packages[packages], which can contain stored procedures and functions.
...
In Jaybird 5 and earlier, it was not possible to execute the https://firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref50/fblangref50-transacs.html#fblangref50-transacs-statements[transaction management statements] `COMMIT`, `ROLLBACK` (without `RETAIN` or a savepoint) and `SET TRANSACTION`.
...
This JDBC escape has been implemented in Jaybird 6.0.5 (and backported to Jaybird 5.0.12).
...
* Improvement: Setting oversized strings on `CHAR` or `VARCHAR` parameters with character set UTF8 will now throw a `DataTruncation` exception on `setString` (and not set a value) instead of a `SQLException` with a "`string right truncation`" error on execute (issue:[396])
+
...
* Improvement: Implemented `java.sql.Connection.abort(Executor)` to forcibly abort a connection (issue:[496])
+
...
* Changed: `DatabaseMetaData.getTypeInfo()` column `FIXED_PREC_SCALE` (column 11) now returns `false` for all data types except `NUMERIC` and `DECIMAL` (issue:[551])
+
...
* Consistent use of `BigDecimal.valueOf(double)` instead of a combination of `new BigDecimal(double)` and `BigDecimal.valueOf(double)` (issue:[553])
* Switched blob identification in message BLR from `blr_quad` to `blr_blob2` (issue:[726])
* Removed `finalize()` methods and -- where it made sense -- replaced them with use of `Cleaner` (issue:[727])
...
* Fixed formatting of `isc_formatted_exception` to not repeat the original parameters of the exception (issue:[749])
* Added aliases `ApplicationName` and `applicationName` for connection property `processName` (issue:[751])
* Fixed: on `CHAR` fields, a too short value could be returned if the string contained one or more codepoints represented by surrogate pairs and the string length in `char` exceeded the maximum string length (issue:[760])
+
...
* Improvement: Do not reject attempts to read blob id 0 (issue:[764])
+
...
* Fixed: The implementation of `Blob.getBytes(long, int)` threw a `SQLException` if the remaining bytes of the blob were less than the requested number of bytes (issue:[767])
+
...
* Fixed: `CallableStatement.getXXX(String)` could return value from wrong column (issue:[771])
+
...
* Fixed: FBRowUpdater incorrectly considers result set with only partial PK updatable (issue:[780])
+
...
* The user manager API (`UserManager`/`FBUserManager` and `User`/`FBUser` in package `org.firebirdsql.management`) has been deprecated (issue:[782])
+
...
* Fixed: Use of offset timezone names (e.g. `+05:00`) for `sessionTimeZone` would result in a warning being logged, and an incorrect conversion applied (in UTC instead of the offset) when using the legacy time types (issue:[786])
+
This change was also backported to Jaybird 5.0.4.
* New feature: Added column `JB_PK_INDEX_NAME` to the result set of `DatabaseMetaData.getPrimaryKeys` with the name of the index backing the primary key (issue:[791])
+
...
+
This improvement was backported to Jaybird 5.0.5.
* Improvement: Added column `JB_PK_INDEX_NAME` and `JB_FK_INDEX_NAME` to the result set of to `getImportedKeys`, `getExportedKeys` and `getCrossReference` of `DatabaseMetaData` with the names of the index backing the primary key and foreign key (issue:[798])
+
Given this is a non-standard extension, it is advisable to retrieve these columns by name, not by position.
* Change: `TYPE_FORWARD_ONLY` is no longer upgraded to `TYPE_SCROLL_INSENSITIVE` when requesting holdable result sets explicitly (holdability `HOLD_CUSORS_OVER_COMMIT`) or implicitly (`defaultResultSetHoldable=true` when not specifying holdability) (issue:[802])
+
See also <<compat-rs-fw-only-holdable>>.
* Improvement: `isPoolable()` on `PreparedStatement` and `CallableStatement` now returns the default of `true` -- `Statement` returns `false` -- as required by JDBC, and `setPoolable` records the value set to be returned by `isPoolable` (issue:[803])
+
...
* The state of `Connection.setReadOnly(boolean)` was not retained after calling `Connection.setTransactionIsolation(int)` or other method calls that changed the current transaction parameter buffer (issue:[805])
...
* Fixed: `ResultSet.isBeforeFirst()` and `ResultSet.isAfterLast()` should always report `false` for an empty result set (issue:[808])
...
* Improvement: `setObject`/`updateObject` methods on `PreparedStatement`, `CallableStatement` and `ResultSet` with the `int scaleOrLength` parameter will now redirect to variants accepting a length of `set/updateBinaryStream` for `InputStream` and `set/updateCharacterStream` for `Reader` (issue:[822])
* New feature: Reporting of ``SQLWarning``s can be disabled with connection property `reportSQLWarnings` (supported case-insensitive values: `ALL` (default), `NONE`) (issue:[825])
+
...
* `org.firebirdsql.gds.ng.wire.version__NN__` (with _NN_, is `10`, `11`, `12`, `13`, `15`, `16`, `18`, and `19`)
* `org.firebirdsql.jaybird`
...
[#breaking-changes-internal-api-6-0-6]
==== Breaking changes internal API for 6.0.6

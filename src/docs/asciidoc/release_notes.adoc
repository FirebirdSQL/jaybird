...
. If you use the NATIVE or EMBEDDED protocols, add the following JARs to the classpath
** `jaybird-native-{version_wo_target}.jar`
** `jna-{jna-version}.jar` (if you have an older version of JNA, replace it)
. To enable "`ChaCha64`" wire encryption support add the following JARs to the classpath
...
If you use native or embedded connections using Jaybird, you will need to explicitly add the appropriate Maven dependency (or `jaybird-native-{version_wo_target}.jar` and `jna-{jna-version}.jar`) to your classpath.
...
Properties in the `SYSTEM` context are read-only.
...
All JDBC-specified properties _get_/_set_ in `USER_SESSION`.
...
If you want to use the Type 2 driver, or Firebird embedded, then you need to include `jaybird-native-{version_example}` and `jna-{jna-version}.jar` on the classpath.

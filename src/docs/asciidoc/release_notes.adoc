...
+
As part of this change, queries were changed to convert the `RDB$TRANSACTION_DESCRIPTION` to an octets varchar field (to avoid additional roundtrips for blob fields), and -- when applicable -- only query transactions that have a transaction description written by Jaybird
+
...
* Change: Select statements no longer produce an update count (https://github.com/FirebirdSQL/jaybird/issues/703[jaybird#703])
+
Jaybird will no longer report an update count of `0` after `getMoreResults()` on a statement that executed a select, but immediately report `-1` (which it previously only did after a second call to `getMoreResults()`).
+
Other statements that produce result sets, like `EXECUTE BLOCK` with `SUSPEND` and Firebird 5.0 or higher DML with a `RETURNING` clause, will still report an update count.

...
See also <<java-support,Java support>> in <<whats-new-in-jaybird-5,What's new in Jaybird 5>>.
...
* New Feature: `org.firebirdsql.managemen.FBTableStatisticsManager` to retrieve the table execution statistics of a connection (https://github.com/FirebirdSQL/jaybird/issues/716[jaybird#716])
+
...
*The list might not be complete, if you notice a difference in behavior that is not listed, please report it as bug.*
...
* `FBCachedClob(FBCachedBlob, String)`, use `FBCacheClob(FBCachedBlob, FBBlob.Config)`
...
* `BlobLenghtProcessor` is now final
...
The OOREMOTE protocol (JDBC URL prefix `jdbc:firebird:oo` and `jdbc:firebirdsql:oo`) are deprecated in Jaybird 5 and will be removed in Jaybird 6.

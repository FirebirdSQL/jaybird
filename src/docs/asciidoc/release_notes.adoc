...
* Fixed: Legacy authentication fails with non-ASCII characters in password (issue:[931])
+
...
* Two system properties were added to disable forced close of a connection after fatal errors (issue:[934])
+
See also <<disable-force-close-on-fatal>>.
+
This change was backported from Jaybird 6.0.6.
* Fixed: Off-by-one error in default value for `maxInlineBlobSize` (issue:[939])
+
...
* Dependency update: updated `net.java.dev.jna:jna` from 5.18.1 to 5.19.1 (used by native and embedded) (issue:[952])
* Made changes to address various potentially confusing warnings and errors logged during finalization (issue:[953])
+
Some of these changes are similar as already implemented in Jaybird 6 (but still using finalizers).
+
...
** Finalization was removed from `AbstractFbTransaction` and added to `JnaTransaction` (native).
+
Given active transactions are normally only eligible for GC if the connection itself is eligible for GC, the attempt to roll back an active transaction from its finalizer is unnecessary (closing the connection already results in a server-side rollback).
This could also result in warnings logged about failures to roll back as the underlying connection could already have been closed.
+
For `JnaTransaction`, the new finalizer will only attempt to disconnect the transaction with `fb_disconnect_transaction`.
This is similar to what Jaybird 6 already does.
** Finalization was removed from wire protocol implementations of `FbDatabase` and `FbService`.
The close (e.g. by finalization) of the underlying socket is now considered sufficient.
+
This is not a clean close and may result in "`connection reset by peer`" errors (e.g. error 10054 on Windows, error 104 on Linux) logged in `firebird.log`.
** Finalization was removed from `V10AsynchronousChannel` (used for events).
The close (e.g. by finalization) of the underlying socket is now considered sufficient.
** Finalization of `JnaDatabase` (native) now only performs a cancel with `fb_cancel_abort`.
** Finalization of `JnaStatement` (native) now only frees the connection handle.
* The GPG key used to sign releases on Maven Central and GitHub was changed to key-id 373F 65A9 F373 1D5C (fingerprint 8372 B5DA 39C9 2FE1 DB4B BB62 373F 65A9 F373 1D5C).
This is a 4096-bit key, where the previous one was 2048 bit.
...
* JDBC 4.5 support: `JaybirdTypeCodes.DECFLOAT` and `JaybirdType.DECFLOAT` now use type code `2015` instead of `-6001` (issue:[917])
+
Applications referencing these constants may need to be recompiled to function correctly with Jaybird 5.0.12 and higher.
If you use Java 26 or higher, we recommend using `java.sql.Type.DECFLOAT` and `java.sql.JDBCType.DECFLOAT` instead.
* JDBC 4.5 support: implemented methods `enquoteIdentifier`, `enquoteLiteral`, `enquoteNCharLiteral`, and `isSimpleIdentifier` on `FBConnection`, and added them to interface `FirebirdConnection` for access in older Java versions (issue:[918])
+
...
* JDBC 4.5 support: implemented "`disable escape processing`" JDBC escape (`++{\...\}++`) (issue:[920])
+
See also <<jdbc-escape-disable-proc>>.
* Fixed: JDBC escapes should not be parsed inside dialect 3 delimited identifiers or dialect 1 string literals (issue:[922])
* Fixed: `IndexOutOfBoundsException` in `FBCachedBlob.getBytes(long, int)` for position or length beyond end of data (issue:[924])
* Fixed: Using native client, password is limited to 255 bytes (issue:[926])
* Fixed: Infinite loop in `FBPooledConnection#fireConnectionError(SQLException)` if the exception has a chained exception and neither is fatal (issue:[928])
* JDBC 4.5 support: `FBDatabaseMetaData.getJDBCMinorVersion()` should report 5 (for JDBC 4.5) on Java 26 and higher (issue:[929])
...
* Backported fatal error detection improvements for `FBPooledConnection` from Jaybird 6 (issue:[899])
* Fixed: `FBServiceManager.getAuthPlugins()` reported the `dbCryptConfig` value (issue:[902])
+
This fix was backported from Jaybird 6.0.4.
* Fixed: `Statement.cancel()` causes lockup (issue:[904])
+
This is a partial backport from Jaybird 7.
See also <<breaking-changes-internal-api-5-0-11>>.
* Fixed: Incomplete detection of fatal connection errors for deferred actions (issue:[905])
+
...
+
This fix was backported from Jaybird 6.0.4.
* Dependency update: updated `net.java.dev.jna:jna` from 5.17.0 to 5.18.1 (used by native and embedded protocols) (issue:[911])
* Fixed: `FBDatabaseMetaData.getJDBCMinorVersion()` should report 4 (for JDBC 4.4) on Java 24 and higher (issue:[914])
...
* Fixed: Negative buffer size on Firebird 2.5 if information response is 32KiB or greater (issue:[894])
+
...
* Fixed: `ResultSet` move incorrectly closes input `Clob` (issue:[881])
...
* Improvement: added `authPlugins` property on `FBManager` (issue:[865])
+
...
* Improvement: increased default and maximum SQL info sizes used for retrieving statement information like columns, parameters and plan information (issue:[868])
+
--
...
* Improvement: backported inline blob support (Firebird 5.0.3 and higher) from Jaybird 7 (issue:[871])
+
See also <<blob-performance-inline-blob>>.
* Improvement: The time zone mapping was updated (issue:[875])
...
* Improvement: backported deferred blob open optimization from Jaybird 7 (issue:[841])
+
See also <<blob-performance-defer-open>>.
* Fixed: `NullPointerException` in `getGeneratedKeys()` with blob columns after (auto)commit (issue:[846])
+
...
* Fixed: Fetch response with status=0 (FETCH_OK) and count=0 was logged on DEBUG as an unexpected response (issue:[848])
+
This fix was backported from Jaybird 6.0.1.
* Improvement: backported performance improvements for blob reading and writing from Jaybird 6 (issue:[850])
+
For details see:
+
--
...
--
* Improvement: backported fetching all known blob info items on open from Jaybird 7 (issue:[852])
+
See also <<blob-performance-info-items>>.
* Dependency update: updated `net.java.dev.jna:jna` from 5.15.0 to 5.17.0 (used by native and embedded protocols) (issue:[855])
...
* Fixed: Exceptions during statement execution did not always complete the statement, which could delay transaction commit in auto-commit mode (issue:[806])
+
This fix was backported from Jaybird 6.
* Fixed: Closing a connection when the database was shutdown, or the connection was otherwise broken, could result in a `NullPointerException` (issue:[812])
+
This fix was backported from Jaybird 6.
* Fixed: Error "`Column unknown; IND.RDB$CONDITION_SOURCE`" when calling `DatabaseMetaData.getIndexInfo` on Firebird 5.0 with a Firebird 4.0 (ODS 13.0) database (issue:[813])
+
...
* Fixed: Calling `ResultSet.wasNull()` when on the insert-row throws a `SQLException` instead of reporting the null-state of the last retrieved column (issue:[816])
+
...
* Fixed: Inserting a row into a result set backed by a server-side scrollable cursor could include the inserted row twice (issue:[819])
+
...
* Improvement: Updated JNA dependency to version 5.15.0 (issue:[823])
+
...
* Fixed: `FBResultSetMetaData.getPrecision` would always estimate the precision of `NUMERIC` or `DECIMAL` columns instead of obtaining the actual precision if the column position was 71 or higher (issue:[731])
+
This fix was backported from Jaybird 6.
* Optimized the query to retrieve extended field info for `ResultSetMetaData.getPrecision` to only retrieve columns of type `NUMERIC` or `DECIMAL` (issue:[732])
+
This improvement was backported from Jaybird 6.
* Fixed: `PreparedStatement.executeBatch()` of statement without parameters throws "`Statement used in batch must have parameters [SQLState:07001, ISC error code:335545186]`" on Firebird 4.0 or higher (issue:[788])
+
...
* New feature: `ResultSetMetaData.isAutoIncrement(int)` reports `true` for identity columns *if* Jaybird can identify the underlying table and column (issue:[793])
+
This feature was backported from Jaybird 6.
* New feature: Boolean connection property `extendedMetadata` (default `true`) to disable querying of extended metadata for `getPrecision(int)` and `isAutoIncrement(int)` of `ResultSetMetaData` (issue:[795])
+
...
+
This feature was backported from Jaybird 6.
* Improvement: The `FILTER_CONDITION` of `DatabaseMetaData.getIndexInfo` is populated for Firebird 5.0 partial indices (issue:[797])
+
This improvement was backported from Jaybird 6.
* Fixed: `ResultSet.isBeforeFirst()` and `ResultSet.isAfterLast()` should always report `false` for an empty result set (issue:[807])
* Improvement: `Statement.getResultSet` no longer throws a `SQLException` with message "`Only one result set at a time/statement`" if the current result set has already been returned by `executeQuery` or a previous call to `getResultSet` (issue:[762])
+
...
+
This improvement was backported from Jaybird 6.
* Improvement: Updated JNA dependency to version 5.14.0 (issue:[810])
+
...
* Fixed: Potential NPE when `warningMessageCallback` is `null` while reading operations or consuming packets (issue:[778])
+
...
* Fixed: FBRowUpdater incorrectly considers result set with only partial PK updatable -- backported from Jaybird 6 (issue:[780])
+
...
* Fixed: Use of offset timezone names (e.g. `+05:00`) for `sessionTimeZone` would result in a warning being logged, and an incorrect conversion applied (in UTC instead of the offset) when using the legacy time types -- backported from Jaybird 6 (issue:[787])
...
* Improvement: Do not reject attempts to read blob id 0 -- backported from Jaybird 6 (issue:[765])
+
...
* Fixed: on `CHAR` fields, a too short value could be returned if the string contained one or more codepoints represented by surrogate pairs and the string length in `char` exceeded the maximum string length -- backported from Jaybird 6 (issue:[770])
+
We now truncate the returned string if the codepoint count exceeds the maximum string length.
* Fixed: `CallableStatement.getXXX(String)` could return value from wrong column -- backported from Jaybird 6 (issue:[772])
...
* Fixed: Reconnect transaction with a transaction id exceeding 0x7FFF_FFFF did not work (issue:[734])
* New feature: add connection property `parallelWorkers` to set Firebird 5.0 `isc_dpb_parallel_workers` (issue:[737])
* New feature: add `MaintenanceManager.upgradeOds()` for the Firebird 5.0 gfix/service repair action to perform a minor ODS upgrade of a database (issue:[738])
* New feature: add parallel workers support for `BackupManager` (issue:[739])
* New feature: add parallel workers support for _sweep_ in `MaintenanceManager` (issue:[740])
* Fixed: `DatabaseConnectionProperties.setServerBatchBufferSize(int)` ignored provided value and always set default (`0`, or "`use server-side maximum`") (issue:[741])
+
...
* New feature: add `MaintenanceManager.fixIcu()` for the Firebird 3.0 gfix/service repair action "`ICU`" to update or rebuild collations and indexes when the ICU version changed (issue:[744])
* Fixed: The first call to `getTableStatistics()` of a `FBTableStatisticsManager` instance returned only a few or even no tables; if no tables were returned, subsequent calls would also return no tables (issue:[747])
+
...
* Fixed: Executing DML with a RETURNING clause containing a blob column would return the blob-id instead of the blob value (issue:[728])
* Fixed: `CallableStatement.getString` and `CallableStatement.getObject` would incorrectly trim string values (issue:[729])
* Fixed: `ResultSetMetaData.getPrecision(int)` of a connectionless result set could throw a `NullPointerException` if the column was of type `FLOAT` or `DOUBLE PRECISION` (issue:[730])
...
--
+
This is not something that we can fix or change in a later Jaybird release.
...
Jaybird {version_wo_target} was tested against Firebird 2.5.9, Firebird 3.0.14, Firebird 4.0.7, and Firebird 5.0.4, but should also support other Firebird versions from 2.5 and up.
...
Jaybird 5 supports Java 8 (JDBC 4.2), and Java 9 and higher (JDBC 4.3/JDBC 4.4/JDBC 4.5).
...
For Jaybird {version_wo_target} that means we support Java 8, Java 11, Java 17, Java 21, Java 25, and Java 26.
...
The Java 8 builds have all JDBC 4.3 - JDBC 4.5 related functionality and can be used on Java 9 and higher as well, but the Java 11 version has additional features, like the ChaCha wire encryption.

Jaybird 5 is not modularized, but all versions declare the automatic module name `org.firebirdsql.jaybird`.
If you need "`real`" module support, upgrade to https://firebirdsql.org/en/jdbc-driver/#jaybird6[Jaybird 6].
...
Since Jaybird 5.0.12 (and Jaybird 6.0.5), Jaybird is compatible with the specified changes and new features.
...
.JNA dependency for native and embedded
[source,groovy,subs="verbatim,attributes"]
----
...
The implementation relies on the `ChaCha20` cipher introduced by https://openjdk.org/jeps/329[JEP 329^].

Jaybird 5 does not support the `ChaCha64` wire protocol encryption.
...
See also https://github.com/FirebirdSQL/jaybird/blob/master/devdoc/jdp/jdp-2021-04-real-scrollable-cursor-support.md[jdp-2021-04].
...
Since Jaybird 5.0.13 (and Jaybird 6.0.6), Jaybird supports two new Boolean system properties.
...
* Fixed: changes to the transaction configuration (transaction parameter buffer configuration) of one connection are no longer propagated to other connections with the same connection properties (issue:[428])
+
Backported to Jaybird 3.0.9 and 4.0.1.
* Changed: build migrated from Ant to Gradle (issue:[461])
* Changed: removed debug logging in `AbstractFbStatement.ensureClosedCursor` (issue:[604])
+
...
* New feature: Support for NBackup GUID-based backup and in-place restore (issue:[672])
+
...
* New feature: Support for NBackup fixup and preserve sequence (issue:[673])
+
...
* Improvement: The `dbCryptConfig` connection property now supports _base64url_ (the "`URL and Filename safe`" Base 64 Alphabet) with the `base64url:` prefix (issue:[677])
* Fix: `FBStatement` allowed retrieval of update count after error, while `FBPreparedStatement` -- correctly -- did not (issue:[681])
+
Now, after an exception, attempting to retrieve the update count with `getUpdateCount()` will return `-1`.
* Improvement: Renamed `NativeResourceUnloadWebListener` to `NativeResourceUnloadWebListenerJavaX` and added its twin `NativeResourceUnloadWebListenerJakarta` using the `jakarta.servlet` namespace to support native resource unloading on Jakarta EE 9 and higher (issue:[684])
* Improvement: `Connection.isValid(int)` now asynchronously checks validity of connections, allowing the timeout to also be honoured for native connections (issue:[685])
* New feature: Support for `isc_spb_expected_db` on service manager (issue:[691])
+
...
* Change: Removed finalization from `FBConnection` and `FBStatement` (issue:[699])
* Fix: XAResource checked at most 10 records for forget or recovery operations (issue:[701])
+
...
* Change: Stop reporting `0` update count with `getUpdateCount()` for statement types that never produce an update count (issue:[703])
+
...
* New feature: Support for role name on FBManager (issue:[705])
* New feature: Support for NBackup "`clean history`" option (issue:[706])
+
...
* Improvement: Allow statements longer than 64KB with native and embedded connections (issue:[713])
+
...
* Fixed: `DatabaseMetaData.getIdentifierQuoteString()` should return `" "` (space) for connection dialect 1 (issue:[714])
...
* Improvement: Populate `COLUMN_DEF` of `DatabaseMetaData.getProcedureColumns` with the default of the parameter (issue:[715])
* Change: Jaybird no longer registers a `SQLWarning` nor logs a warning when connecting without an explicit connection character set (issue:[717])
...
The maximum server-side batch buffer should be sufficient to have a batch of around 4000 rows with the maximum row size (around 64KB in Firebird 5.0 and earlier), more rows are possible for smaller row sizes.
...
Other breaking changes to the internal API in Jaybird 5.0.11 unrelated to the transmit lock:
...
Jaybird 6 will drop support for Firebird 2.5 (see also https://github.com/FirebirdSQL/jaybird/blob/master/devdoc/jdp/jdp-2021-03-drop-firebird-2-5-support.md[jdp-2021-03: Drop Firebird 2.5 support]).
...
Jaybird 6 will drop support for Java 8 and 11, making Java 17 the baseline version (see also https://github.com/FirebirdSQL/jaybird/blob/master/devdoc/jdp/jdp-2022-03-java-17-minimum-version.md[jdp-2022-03: Java 17 minimum version]).

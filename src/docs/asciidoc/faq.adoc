...
Jaybird 4 supports Java 7, Java 8, Java 11, Java 17, and Java 21footnote:[Jaybird 4 was not tested against Java versions after Java 21].
...
Support for Java 9 and higher is limited to Java 11 and 17, but in practice Jaybird should work on all Java 9+ versions up to Java 17footnote:[Jaybird 3 was not tested against Java versions after Java 17].
...
Full Firebird 5.0 support is available in Jaybird 5 and higher.
...
Jaybird 3.0.2 introduces the system property `org.firebirdsql.jdbc.requireConnectionEncoding`, which -- when set to `true` -- will reject connections without a character set (which was the default behaviour in Jaybird 3.0.0 and 3.0.1).
...
In Jaybird 3.0.2 and higher this error will only be thrown if system property {jaybird-manual}#systemproperties-charset[`org.firebirdsql.jdbc.requireConnectionEncoding`] has been set to `true`.
...
With Jaybird 3.0.0 - 3.0.3 connecting to Firebird 3.0 or higher, this usually means that the setting {firebird-conf-ref}#fbconf-wire-crypt[`WireCrypt`] is set to its (default) value of `Required`.
...
With Jaybird 2.2.x connecting to Firebird 3.0 or higher, this usually means that the setting {firebird-conf-ref}#fbconf-wire-crypt[`WireCrypt`] is set to its (default) value of `Required`.
...
With Jaybird 2.2 connecting to Firebird 3.0 or higher, this means that the setting {firebird-conf-ref}#fbconf-auth-server[`AuthServer`] does not include the `Legacy_Auth` plugin.
...
With Jaybird 4 and higher, this can also mean that none of the default authentication plugins, or those specified using connection property {jaybird-manual}#ref-auth-plugins-config[`authPlugins`], are listed in the `AuthServer` setting.
...
Actively supported Jaybird versions -- Jaybird 5 and Jaybird 6 -- support the following JDBC Specification versions:
...
No -- for Jaybird 4 and older, Jaybird 5.0.11 and older, and Jaybird 6.0.4 and older.

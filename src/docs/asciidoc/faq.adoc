...
:toc: left
:icons: font
:jaybird6-full-version: 6.0.6
:jaybird6-jna-version: 5.19.1
:jaybird5-full-version: 5.0.13
:jaybird5-template-version: {jaybird5-full-version}.<java>
:jaybird5-example-version: {jaybird5-full-version}.java11
:jaybird5-jna-version: 5.19.1
...
It is _the_ way to connect to Firebird from Java and other Java Virtual Machine (JVM) based languages.
...
We recommend using your favourite build tool -- like https://maven.apache.org/[Maven^] or https://gradle.org/[Gradle^] -- to get Jaybird from Maven Central instead of downloading it manually from our site or GitHub.
...
If you want to use native or embedded (JDBC Type 2), add `jaybird-native` as a dependency:
...
To enable "`ChaCha64`" wire encryption support for pure Java connections, add `chacha64-plugin` as a dependency:
...
Contrary to Jaybird 4, the Maven relocation artifacts `jaybird-jdk17` for Java 7 and `jaybird-jdk18` for Java 8 are no longer provided.
...
If you want to use native or embedded (JDBC Type 2), you need to explicitly include JNA {jaybird5-jna-version} as a dependency:
...
All Jaybird <<where-do-i-get-jaybird,distribution ZIPs>> contain a `jaybird-<version>-sources.zip` with the Java sources used for that specific version.
The Maven Central repository also has this `jaybird-<version>-sources.zip`, and most IDEs usually offer to download it when you navigate to any Jaybird class.
...
See <<error-isc-connect-reject>> for a workaround.

[discrete#firebird-jaybird-5]
=== Jaybird 5
...
This is best suited for client-server applications with a dedicated database server.
Port can be omitted (default value is `3050`).
Host name can be omitted (default value is `localhost`).
...
A database located on `/opt/firebird/db.fdb` should use (note the double slash after the port!):

[listing]
----
jdbc:firebird://myserver:3050//opt/firebird/db.fdb
----
...
See also <<native-fbclient-connection-strings>>.
...
[#native-fbclient-connection-strings]
==== fbclient connection strings

Since Jaybird 5, if the string after `jdbc:firebird:native:` does not start with `//`, it is forwarded to the client library for parsing.
Valid connection strings -- including the "`legacy URL format`" -- are determined by the version of _fbclient_ that is loaded.

In other words, the actual format is:

[listing]
----
jdbc:firebird:native:<fbclient-connection-string>
jdbc:firebirdsql:native:<fbclient-connection-string>

<fbclient-connection-string>> ::=
  !! Anything that does not start with '//' !!
----

Some examples that are also valid (assuming a Firebird 3.0 or higher `fbclient` is used):
...
NOTE: Support for the WNET protocol was removed in Firebird 5.0.
...
[NOTE]
====
In practice, embedded connection strings accept the same <<native-fbclient-connection-strings>> as documented for _native_.

The only real differences with _native_ are:

. If a Firebird Embedded Provider library is on the classpath, it will try to load it
. It will try to load _fbembed_ before _fbclient_
. URLs starting with `//` are not parsed and forwarded to the native client as is
====
...
To login, you must surround the username with double quotes so it's handled case-sensitive.
...
Make sure you check the other settings mentioned in that article, otherwise you'll get another error (<<error-isc-login-error>>).
...
The caveats for <<jdbc44-support,JDBC 4.4>> also apply to JDBC 4.5.

...
public final class InlineBlob implements FbBlob {
    ...
    public void seek(int offset, SeekMode seekMode) throws SQLException {
        try (LockCloseable ignored = withLock()) {
            ...
            switch (seekMode) {
            case ABSOLUTE:
                position = Math.min(Math.max(0, offset), data.length);
                break;
            case ABSOLUTE_FROM_END:
                position = Math.max(0, Math.min(data.length + offset, data.length));
                break;
            case RELATIVE:
                position = Math.max(0, Math.min(position + offset, data.length));
                break;
            }
        } catch (SQLException e) {
            errorOccurred(e);
    ...
    private SQLException writeNotSupported() {
        SQLException e = new FbExceptionBuilder()
                .nonTransientException(ISCConstants.isc_segstr_no_write).toSQLException();
        errorOccurred(e);
        return e;
    }

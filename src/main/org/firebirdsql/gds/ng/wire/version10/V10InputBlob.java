...
import static org.firebirdsql.util.ByteArrayHelper.emptyByteArray;
...
public class V10InputBlob extends AbstractFbWireInputBlob implements FbWireBlob, DatabaseListener {
    ...
    public byte[] getSegment(final int sizeRequested) throws SQLException {
        try {
            ...
            return extractGetSegmentResponse(response.getData());
        } catch (SQLException e) {
            errorOccurred(e);
            throw e;
        }
    ...
    private int extractGetSegmentResponse(byte[] b, int off, int len) throws SQLException {
        try {
            ...
            xdrIn.skipFully(8); // blob-id (unused)
            ...
            if (bufferLength > 0) {
                ...
                while (bufferRemaining > 2) {
                    ...
                    if (segmentLength > bufferRemaining) {
                        throw new IOException(String.format(
                                ...
                                segmentLength, bufferRemaining));
                    } else if (segmentLength > len - count) {
                        throw new IOException(String.format("Returned segment length %d exceeded remaining size %d",
                ...
                xdrIn.skipFully(bufferRemaining);
                // Skip buffer padding

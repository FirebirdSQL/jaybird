...
import static org.firebirdsql.gds.ISCConstants.isc_net_write_err;
...
public abstract class AbstractFbWireBlob extends AbstractFbBlob implements FbWireBlob {
    ...
    protected final void sendOpen(BlobOpenOperation openOperation, boolean flush) throws SQLException {
        try {
            ...
            throw new FbExceptionBuilder().nonTransientConnectionException(isc_net_write_err).cause(e).toSQLException();
        }
    }
    ...
    protected final void receiveOpenResponse() throws SQLException {
        try {
            ...
            throw new FbExceptionBuilder().nonTransientConnectionException(isc_net_read_err).cause(e).toSQLException();
        }
    }
    ...
    protected void closeImpl() throws SQLException {
        closeOrCancelImpl(WireProtocolConstants.op_close_blob);
    }

    @Override
    protected void cancelImpl() throws SQLException {
        closeOrCancelImpl(WireProtocolConstants.op_cancel_blob);
    }

    private void closeOrCancelImpl(int releaseOperation) throws SQLException {
        try {
            releaseBlob(releaseOperation);
        } finally {
            releaseResources();
        }
    }
    ...
    private void deferredExceptionHandler(Exception exception) {
        if (exception instanceof SQLException) {
            ...
            // NOTE: Intentionally *not* calling AbstractFbBlob.errorOccurred
            exceptionListenerDispatcher.errorOccurred(new FbExceptionBuilder()
                    .nonTransientConnectionException(isc_net_read_err).cause(exception)
                    .toSQLException());
        }
    }

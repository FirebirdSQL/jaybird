...
public class FBProcedureCall implements Cloneable {
    ...
    private static Vector<FBProcedureParam> cloneParameters(final Vector<FBProcedureParam> parameters) {
        final Vector<FBProcedureParam> clonedParameters = new Vector<FBProcedureParam>(parameters.size());
     ...
     * parameter was found.
     */
    public int mapOutParamIndexToPosition(int index, boolean compatibilityMode) throws FBSQLException {
        int position = -1;
    ...
    public String getSQL(boolean select) throws FBSQLException {
        ...
        for (FBProcedureParam param : inputParams) {
            ...
            // to the next one.
            if (!param.isValueSet()) {
                if (param.isParam() && outputParams.size() > 0 && outputParams.get(param.getPosition()) == null)

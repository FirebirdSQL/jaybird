...
* `getCatalogs()` list packages, with package names in `TABLE_CAT`.
...
* `getCatalogSeparator()` will return `"."` (string with period).
* `getCatalogTerm` will return `"PACKAGE"`.
* `isCatalogAtStart()` will return `true`.
* `getMaxCatalogNameLength()` will return 31 or 63 depending on the max identifier length of the database.
* `supportsCatalogsInDataManipulation()` will return `true` (i.e. access selectable stored procedures and functions).
* `supportsCatalogsInProcedureCalls()` will return `true`.
* The other `supportsCatalogsIn__type__()` methods will return `false`.
* Other metadata methods with a `catalog` parameter will continue to ignore it, just like they do when `useCatalogAsPackage` is disabled.
